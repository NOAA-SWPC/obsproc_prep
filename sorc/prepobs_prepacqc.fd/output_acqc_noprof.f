...
c      than "max_reps" merged arcraft reports in the input PREPBUFR file -- in this case no
...
c      PREPBUFR file will be copied to output PREPBUFR file but they will not be QC'd
     ...
     +             'can be QC''d'
              ...
              if(QCdrptsidx.gt.1) then
                if(c_qc(QCdrptsidx-1)(2:2).eq.'R'.or.       ! time reabilitated
     +             c_qc(QCdrptsidx-1)(3:3).eq.'R'.or.       ! latitude reabilitated
     +             c_qc(QCdrptsidx-1)(4:4).eq.'R'.or.       ! longitude reabilitated
     +             c_qc(QCdrptsidx-1)(5:5).eq.'R'.or.       ! pressure/altitude reabilitated
     +             c_qc(QCdrptsidx-1)(6:6).eq.'R'.or.       ! temperature reabilitated
     +             c_qc(QCdrptsidx-1)(5:5).eq.'r') print 65 ! pressure/altitude reabilitated
   65 format(131('^'))
              endif
              if(c_qc(QCdrptsidx)(2:2).eq.'R'.or.           ! time reabilitated
     +           c_qc(QCdrptsidx)(3:3).eq.'R'.or.           ! latitude reabilitated
     +           c_qc(QCdrptsidx)(4:4).eq.'R'.or.           ! longitude reabilitated
     +           c_qc(QCdrptsidx)(5:5).eq.'R'.or.           ! pressure/altitude reabilitated
     +           c_qc(QCdrptsidx)(6:6).eq.'R'.or.           ! temperature reabilitated
     +           c_qc(QCdrptsidx)(5:5).eq.'r') then         ! pressure/altitude reabilitated
                print 61
   61 format(131('v'))
