...
C 2012-03-05  S. MELCHIOR  CODE FURTHER STREAMLINED. CORRECTED 
C       VARIOUS TYPO'S IN LEGACY DOCUMENTATION. ADDED MORE CONCISE
C       DESCRIPTIONS TO LEGACY DOCUMENTATION. ADDED MORE COMPLETE
C       DIAGNOSTIC OUTPUT STATEMENTS FOR INSTANCES WHEN SPECIFIC DO
C       LOOPS ARE OUT OF BOUNDS AND WHEN CONDITIONAL STATEMENTS ARE
C       NOT MET. ADDED MORE CONCISE COMMENTS TO IDENTIFY DO LOOP 
C       BOUNDARIES. CORRECTED A DO LOOP TERMINUS PARAMETER FROM
C       NLEV TO NLV IN SUBROUTINE COMSTAT. ADDED A MISSING RETURN
C       STATEMENT TO SUBROUTINE HT. UPDATED SUBROUTINE DISTR TO BE
C       MORE FLEXIBLE AND UTILIZE NDIV AND NX AS ARRAY LIMITS RATHER
C       THAN THE HARD CODED 23 AND 899, RESPECTIVELY. EXTENDED MSK
C       ARRAY ALLOCATION FROM 899 TO 1000 IN SUBROUTINES INCDIST
C       AND RESDIST TO ALLOW FOR MORE STATION STATISTICS TO BE
C       PROCESSED - ON OCCASION THE 899 LIMIT HAD BEEN MET AND 
C       EXCEEDED CAUSING MEMORY LEAKS. UPDATED SUBROUTINE EVNOUT TO 
C       USE INCOMING ARGUMENT NLV TO DEFINE MEMORY ALLOCATION FOR
C       VARIOUS REAL AND INTEGER ARRAYS. THIS MAKES THE SUBROUTINE
...
C       RESDIST, SELECT, SOLVE, ZTOI, AND ZTRES.  NINC=3 HAS BEEN 
C       ADDED TO COMSTAT, GETDAT, INCDIST, INCR, INIT, MATR, DMA, 
C       RESDIST, SELECT, SOLVE, AND ZTRES.  NEVNT=160000 WAS ADDED TO 
C       DMA AND EVNOUT. NDIV=23 HAS BEEN ADED TO INCDIST AND RESDIST.
C       RENAMED PARAMETER NE TO NME FOR CLARIFICATION SINCE NE IS 
C       DIFFICULT TO SEARCH ON AND IS ALSO USED FOR INEQUALITY
C       COMPARISON TESTING (.NE.).  ADDED LOGIC TO TURN OFF BIRD
C       CHECK ALGORITHM WHEN THE VADWND DATA IS LEVEL2 FORMAT. 
C       INCREASED NRPT FROM 80000 TO 160000 TO ACCOMMODATE LEVEL2 
C       FORMAT VADWND DATA. CORRECTED BUG IN SUBROUTINE GETDAT TO
C       PROPERLY HANDLE SITUATION WHEN SUBSET IS VADWND AND THE 
C       NUMBER OF LEVELS (NUM) EXCEEDS NRPT (160000).
...
C 2012-03-05  S. MELCHIOR  RENAMED NE TO NME FOR EASE OF 
C       NAVIGATION AND CLARIFICATION SINCE NE IS ALSO USED
C       FOR INEQUALITY TESTING (.NE.). INCREASED NEV FROM 80000
C       TO 160000 TO ACCOMMODATE LEVEL2 FORMAT VADWND DATA. 
C
C ATTRIBUTES:
C   LANGUAGE: FORTRAN 90
C   MACHINE:  NCEP WCOSS
...
C       NLEV TO NLV. ALSO ADDED PARAMETERS NTIMES=6 AND NINC=3 TO 
C       TIDY UP THE CODE. RENAMED NE TO NME TO EASE NAVIGATION AND
...
C       DEFINE MEMORY ALLOCATION FOR VARIOUS REAL AND INTEGER
C       ARRAYS. THIS MAKES THE SUBROUTINE MORE ROBUST AND EFFICIENT
C       BY ONLY ALLOTTING THE MEMORY SPACE NECESSARY, NOTHING MORE.
C       ADDED PARAMETER NEVNT=160000 TO TIDY UP THE CODE. CORRECTED  
C       SOME DOCUMENTATION TYPO'S. ADDED DIAGNOSTIC OUTPUT 
C       STATEMENT WHEN LIMITS ARE EXCEEDED. RENAMED NE to NME TO EASE
...
C       WHEN LIMITS ARE EXCEEDED. RENAMED NE TO NME TO EASE
C       NAVIGATION AND FOR CLARIFICATION SINCE NE IS USED FOR 
C       INEQUALITY TESTING (.NE.).  ADDED LOGIC TO TURN OFF BIRD
C       CHECK ALGORITHM WHEN THE VADWND DATA IS LEVEL2 FORMAT.
C       CORRECTED LOGIC TO HANDLE SITUATIONS WHEN THE SUBSET IS
...
c     Turn off bird algorithm QC for level2 vadwnd data
      read(stnid(num),'(i4)',err=89) kdmy
      go to 90 ! radar coded vadwnd data
   89 continue ! level 2 vadwnd data
...
C       STATEMENTS FOR INSTANCES WHEN LIMITS ARE EXCEEDED. RENAMED
C       NE TO NME FOR EASE OF NAVIGATION AND ALSO FOR CLARIFICATION
