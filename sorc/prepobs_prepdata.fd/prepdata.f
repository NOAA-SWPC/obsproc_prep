...
C   GFS, RAP (RAPID REFRESH) AND RTMA NETWORKS.  ALL ANALYSES ARE GSI.
C   ALSO RUNS IN HOURLY NETWORK TO PROCESS GOES CLOUD DATA (ONLY).
...
C    ASDAR AIRCFT FROM PROD AIRCFT ON29 D-SET NOW OBTAINED FROM 1ST
C    BYTE RATHER THAN FROM 2'ND OR 4'TH BYTE FOR TEMP & WIND, RESP.;
C    TEST FOR ASDAR ACFT RPTS LOOKS ONLY FOR 'Z' IN 6'TH BYTE OF STNID
...
C    OVER CONUS TO BE TOSSED IF > 100 ACARS RPTS HERE; STOP 69 ADDED
...
C    BAD/MSG TEMP NO LONGER PROCESS MASS PIECE; ACARS RPTS USE NEW,
C    EXPAND. Q.M., CHGS. SSI/OI Q.M. ON ACARS WIND FROM 2 TO 1
...
C    'IACARL' TO INDICATE FLOOR FOR ACARS; CONV'L/ ASDAR QCACFT WSPD
C    INCR. Q.M. NOW KEPT IN COMMON FOR RPT; RCPT TIME, RPT SUB-TYPE AND
C    QCACFT WSPD INCR. Q.M. (CONV'L/ASDAR AIRCFT WIND RPTS ONLY)
...
C    HOLD INDICATOR FOR MISSING PHASE OF FLIGHT FOR ACFT ASDAR/AMDAR
C    REPORTS
...
C    {HDR(4)} CHGED FROM HRS*100 TO HRS*1000 (CURRENTLY ONLY ACARS CAN
...
C    UNPKED ON29 CAT. 8 C. FIG. 919; FOR AMDAR AND ACARS ACFT: CHAR. 7
C    & 8 OF TRUE STNID NOW READ FROM UNPKED ON29 CAT. 8 C. FIG. 920;
C    FOR ACARS ACFT: RPT. TIME TO NEAREST 1000'TH HOUR NOW READ FROM
...
C    FOR ALL ACFT TYPES AND DEGREE OF TURBULENCE ('DGOT') FOR NON-ACARS
...
C    OF ACARS MOISTURE DATA BUT THEY ARE FLAGGED FOR ALL REPORTS;
...
C     "FLACMS", WHEN TRUE (DEF.) FLAGS ALL ACARS MOISTURE DATA (WAS
...
C     THE 4 ACARS TURBULENCE INDECES AND ENCODES THEM INTO THE PREPBUFR
...
C     IS 18000 M FOR AIREP/PIREP AND ASDAR AND 16500 FOR ACARS); LEVELS
...
C     AVAIL. FOR AMDAR/ASDAR AND ACARS); CORRECTED ERROR INTRODUCED
C     ~1/9/2001 WHICH RESULTED IN NO MASS REPORTS BEING ENCODED INTO
C     THE RUC VERSION OF THE PREPBUFR FILE FOR MOORED AND DRIFTING
C     BUOYS (AFFECTED ONLY THE RUC AND ONLY THESE DATA TYPES); NOW
C     RECOGNIZES THAT W3UNPKB7 CAN RETURN A VAD WIND Q.M. OF 12
C     (MEANING ON REJECT LIST) OR 14 (MEANING SDM PURGED) ALONG
C     WITH EXISTING Q.M. GENERATED FROM RMS VECTOR ERROR; NOW
C     RECOGNIZES THAT W3UNPKB7 CAN RETURN A WIND PROFILER "QUALITY
...
C     ADDED PROCESSING OF E-ADAS ACFT RPTS NOW POSSIBLY INCL. IN
...
C     LOCATION (NOW STORES MISSING); NOW ENCODES ACARS FLIGHT NUMBER
C     (MNEMONIC "ACID") INTO MDCRS ACARS REPORTS; REPLACED CALL TO
C     BUFRLIB ROUTINE "BORT" WITH CALL TO W3LIB ROUTINE ERREXIT, WITH
C     CONDITION CODE 59 IF THERE ARE MORE THAN "MXBLVL" LEVELS IN A
C     REPORT AT THE TIME IT IS ABOUT TO BE ENCODED INTO THE PREPBUFR
C     FILE; ARRAY SUBSKP BUMPED UP FROM (0:12,0:200) TO (0:255,0:200);
C     ADDED NEW IRNMRK VALUE 8 FOR HOURLY (GOES CLOUD DATA) RUNS; RAIN
C     RATE (SSM/I) IS NOW ENCODED WITH STANDARD MNEMONIC REQV INSTEAD
C     OF LOCAL MNEMONIC REQ6 (STILL HIGH PRECISION, BUFRTABLE HAS
C     OPERATOR DESCRIPTORS); ADDED LOGIC IN SUBR. W3FIZZ (CURRENTLY NOT
...
C     COMMENTS IN SUBROUTINE); ADDED PROCESSING OF NASA/MODIS POES
C     AQUA/TERRA SATELLITE-DERIVED WINDS, NOW POSSIBLY INCL. IN
C     "SATWND" DUMP FILE EXISTING DUMP R. TYPE 63, GET PREPBUFR R.
C     TYPES 257 (IR), 258 (W.VAPOR IMGR CLOUD-TOP) AND 259 (W.WAPOR
C     IMGR DEEP LAYER), PROCESSED INTO PREPBUFR FILE UNDER EXISTING TBL
C     A ENTRY "SATWND"; NOW RETURNS 41-WORD (RATHER THAN 2-WORD) ARRAY
C     "OBS2" OUT OF CALL TO IW3UNPBF, WHERE WORDS 3-41 CONTAIN
C     ALTIMETER SETTING (WHICH HAD BEEN RETURNED IN CAT. 8, C.F. 20,
C     NOW ONLY ALTIMETER TBL. VAL. RETURNED FROM HERE), SEA-SURFACE
C     TEMPERATURE, SINGLE-LEVEL SENSIBLE WEATHER ELEMENTS AND SST
C     TABLE VALUE (Q.M.); RETURNS NEW ARRAY ARGUMENTS "OBS3"
C     {(5,MXBLVL,5)} AND "NOBS3" {(5)} OUT OF CALL TO IW3UNPBF WHERE
C     OBS3 CONTAINS MULTIPLE-LEVEL SENSIBLE WEATHER ELEMENTS AND NOBS3
C     DEFINES THE NUMBER OF LEVELS OF DATA IN THE OBS3(X,Y,1) TO
C     OBS3(X,Y,5) ARRAYS (SEE IW3UNPBF DOCBLOCK FOR MORE INFO); ALL NEW
C     PARAMETERS (I.E., SST AND SENSIBLE WEATHER ELEMENTS) IN OBS2 AND
C     OBS3 ARE ENCODED INTO PREPBUFR FILE, BUT ONLY IN THE MASS "PIECE"
C     OF A REPORT (UNLESS THERE IS NO MASS PIECE, THEN THEY ARE ENCODED
C     INTO WIND PIECE; WILL NOW CALC. Q AND Tv IN STOROB IF PQM IS 15
C     (BUTS SETS QQM TO 15) (E.G., FOR DROPs WITH PRESS FAILING D-VAL,
C     BEFORE Q MISSING BUT LATER CALC. IN VIRTMP STEP)
...
C     PRESENT (USED TO FLAG ONLY MDCRS ACARS HERE AND TOSS ALL OTHER
C     TYPES DUE TO CONCERN ABOUT PREPACQC), POINT IS MOOT BECAUSE
C     AIFNOW NOW SET TO TRUE TO PROCESS ALL ACFT TYPE TEMP REPORTS WHEN
C     WIND DATA ARE MISSING; NOW FLAGS ALL ACFT REPORTS BELOW "FLOOR"
C     LEVEL (USED TO FLAG ONLY MDCRS ACARS HERE AND TOSS ALL OTHER
C     TYPES DUE TO CONCERN ABOUT PREPACQC); REPLACED CALL TO BUFRLIB
C     ROUTINE OVRBS1 WITH CALL TO BUFRLIB ROUTINE PKBS1 (OVRBS1
C     OBSOLETE WITH 1/31/2006 VERSION OF BUFRLIB) (THIS IS CURRENTLY
C     NEVER CALLED IN PREPDATA); INCREASED SIZE OF COMMON BLOCK
C     /BITBUF/ ARRAYS IBAY AND MBAY FROM (5000) AND (5000,32) TO
C     (12500) AND (12500,32), RESP. TO MATCH SIZES IN THIS COMMON BLOCK
C     IN 1/31/2006 VERSION OF BUFRLIB; REDEFINES MANY BUFR TABLE
C     DESCRIPTORS FOR TABLE A/D ENTRIES TO AVOID TABLE A/D CONFLICTS
C     WITH OTHER BUFR TABLES; MODIFIED TO PROCESS WINDSAT SCATTEROMETER
...
C     FILE UNDER NEW MESSAGE TYPE "WDSATR); NOW HANDLES SATWINDS FROM
C     NEW INDIAN KALPANA SATELLITE (REPORTS HAVE SUFFIX CHARACTER "K"
C     IN STATION ID); MODIFIED TO HANDLE RETURN OF NEW ARGUMENT
...
C     "RDATA" ARRAY (IN THE HEADER)]; BUMP UP ARRAY SIZE FOR UNPACKED
C     REPORT FROM IW3UNPBF FROM 3000 TO 3500; ACCOUNTS FOR EUMETSAT
C     MSG (METEOSAT-8 AND UP) SATWINDS CONTAINING RFF QUALITY
C     INDICATOR; ADDED READING IN, LISTING AND ENCODING OF PERCENT
C     CONFIDENCE BASED ON - EUMETSAT QUALITY INDEX WITH FORECAST
...
C     AIRFRAME ICING (RECCOS, PIREPS, E-ADAS, CANADIAN AMDAR, TAMDAR),
...
C     TABLE) FOR ACARS REPORTS, ENCODED INTO PREPBUFR FILE (MNEMONIC
...
C
C              **  GROUP "AA" -- NCEP PRODUCTION DATE FILE **
C       (WHERE AA IS IUNIT(1) IN NAMELIST "&PARM"  DESCRIPTION BELOW)
C     UNIT AA   - NCEP PRODUCTION DATE FILE
C
...
C     UNIT EE+1 - 'AIRCAR' BUFR DATA DUMP FILE (MDCRS ACARS AIRCRAFT)
C     UNIT EE+2 - 'AIRCFT' BUFR DATA DUMP FILE (ALL AIRCRAFT OTHER
C                  THAN MDCRS ACARS)
...
C     COND =  29 - ERROR READING NCEP PRODUCTION DATE FILE
...
C              MOISTURE IS FLAGGED PREPBUFR TABLE VALUE=15, ALSO TEMP.
C              PREPBUFR TABLE VALUE IS AT BEST 2, AND VIRT. TEMP. IS
C              NOT ENCODED IN PREPBUFR FILE {APPLIES TO U-AIR CAT.1-5
...
C              HUNDREDTHS OF AN HR) - (MAX IS +/- 12 HRS) (DEFAULT=300.)
CC
C  N O T E -- THE FOLLOWING (2,6) ARRAY REFERS TO THE 2 GEOGRAPHICAL
C              REGIONS & THE 6 AIRCRAFT FLIGHT LVL REPORT TYPES POSSIBLE
...
C                     2 - ASDAR/ACARS REPORTS ORIGINALLY IN AMDAR
C                         FORMAT (HEREAFTER CALLED "AMDAR" REPORTS FOR
C                         SIMPLICITY)
C                     3 - E-ADAS {EUROPEAN AMDAR (ASDAR/ACARS) REPORTS
C                         ORIGINALLY IN BUFR} (HEREAFTER CALLED
C                         "E-ADAS" REPORTS FOR SIMPLICITY)
C                     4 - MDCRS ACARS (HEREAFTER CALLED "ACARS" REPORTS
C                         FOR SIMPLICITY)
C                     5 - TAMDAR (TROPOSPHERIC AIRBORNE METEOROLOGICAL
C                         DATA REPORT) REPORTS FROM GSD MADIS FEED
C                         (HEREAFTER CALLED "TAMDAR" FOR SIMPLICITY)
C                     6 - CANADIAN ASDAR/ACARS REPORTS (HEREAFTER
C                         CALLED "CAN-AMDAR" REPORTS FOR SIMPLICITY)
CC
C    AWINDO - TIME WINDOW (+/-) FOR ACFT RPTS (IN HUNDREDTHS OF AN HR)
C             - (MAX IS +/- 12 HRS)
C                            (DEFAULT - AWINDO(2,6)/7*300.,150.,4*300./)
CC
C    PHIACF - BOUNDARIES OF 6 LATITUDE BANDS FOR AIRCRAFT DATA
C                   (DEFAULT - PHIACF(7)/-90.,-70.,-20.,0.,20.,70.,90./)
C  N O T E -- THE FOLLOWING (6,6) ARRAYS REFER TO THE 6 LATITUDE
C              BANDS IN PHIACF (1ST ELEMENT) AND THE 6 ACFT FLIGHT LVL
...
C                      BUT GIVE ALL VARIABLES A PREPBUFR TBL. VAL. 15
C                                     (DEFAULT - AIRLND(6,6)/36*.TRUE./)
C    JAWIND - AIRCRAFT FLIGHT LEVEL WIND DATA TYPE SWITCH -
C       =    0  PROCESS THIS TYPE
C       = 9999  EXCLUDE THIS TYPE          (DEFAULT - JAWIND(6,6)/36*0/)
C    JAMASS - AIRCRAFT FLIGHT LEVEL MASS DATA TYPE SWITCH -
C       =    0  PROCESS THIS TYPE
C       = 9999  EXCLUDE THIS TYPE       (DEFAULT - JAMASS(6,6)/36*9999/)
CC
C  N O T E -- THE FOLLOWING ARRAYS REFER TO THE 6 ACFT FLIGHT LEVEL
...
C              PREPBUFR TBL. VAL. 15)   (DEFAULT - IACFTL(6)/100,5*2286)
C              (NOTE: 2286 METERS IS 7500 FEET)
...
C                            (DEFAULT - IACFTH(6)/3*18000,16500,2*18000)
C              (NOTE: 16500 METERS IS APPROXIMATELY 95 MB), AND
...
C                                        (DEFAULT - AIFNOW(6)/6*.FALSE.)
C    FLACMS - FLAG AIRCRAFT MOISTURE (SET SPEC. HUMIDITY Q.M. TO 15)?
C              FLACMS =.TRUE.  ---> YES  (DEFAULT - FLACMS(6)/6*.TRUE./)
C              FLACMS =.FALSE. ---> NO
C          N O T E:  CURRENTLY ONLY ACARS AND TAMDAR REPORTS HAVE
C                    MOISTURE
CC
...
C       IUNIT(1)  - UNIT NUMBER FOR NCEP PRODUCTION DATE FILE
...
C                         TO 10**1)
C                      2 FOR CLOUD AMOUNT (OBS. IN PER CENT PRECISE
C                         TO 10**0)
...
C  IN INTERFACE WITH SUBROUTINE IW3UNPBF
     ...
     $ KSKSWD(10),RFFL,QIFY,QIFN,EEQF
      COMMON/ACFTSW/AWINDO(2,6),AIRLND(6,6),JAWIND(6,6),JAMASS(6,6),
     $ IACFTL(6),IACFTH(6),PHIACF(7),KEPAC(6,6),AIFNOW(6),POAF,TRBX(5),
     $ FLACMS(6),PCAT,ROLF,aMSTQ
     ...
     $ JSURFM(11),JSURFW(11),FWINDO(11),PFRALT
     ...
     $ KSKSWN,KSKACF,KNDCDE(3),KSKCLS,KSKTHN,KSKOMR(2),KSKALM(6),
     $ KSKAHM(6),KSKNOW(6),KSKDRP
     ...
     $  'ACFT: FLIGHT-LVL AMDAR, E-ADAS or SUP-OB',  ! 231
     $  'UAIR: RECONNAISSANCE OR DROPWINSONDES   ',  ! 232
     $  'ACFT: FLIGHT-LEVEL ACARS                ',  ! 233
     ...
     $  'ACFT: FLIGHT-LVL AMDAR, E-ADAS or SUP-OB',  ! 131
     $  'UAIR: RECONNAISSANCE OR DROPWINSONDES   ',  ! 132
     $  'ACFT: FLIGHT-LEVEL ACARS                ',  ! 133
     ...
     $ MODPRT,LEVPM,LEVST,LEVQQ,TDLIM,AWINDO,RWINDO,UWINDO,JSURFM,
     ...
     $ FLDMFR,WRMISS,SKGP45,JPASCD,IAWNDO
      NAMELIST/PARM/IUNIT
...
C    CARDS FILE, JUST AFTER NAMELIST TASK, BY THE MAKE_PREPBUFR SCRIPT)
  ...
  322 FORMAT(//53X,'TABLE OF UNIT NUMBERS'/31X,
     ...
     $ 36X,'NCEP PRODUCTION DATE IN UNIT ........................ ',I5/
...
C READ PRODUCTION DATE
      READ(IUNIT(1),'(6X,I10)',END=326)  IDAT10
C SPLIT PRODUCTION DATE INTO COMPONENTS (IDATE)
  ...
  324 FORMAT(//23X,'ALL INPUT DATA SETS WILL BE CHECKED AGAINST THE ',
     $ 'NCEP PRODCUTION DATE: ',I4.4,2('-',I2.2),1X,I2.2,'Z'/)
      CALL W3LOCDAT(IDAT)
...
C  THAT LAST INCOMPLETE BUFR MESSAGE SHOULD BE WRITTEN OUT
 ...
 1951 FORMAT(/,35X,
     ...
     $ 'NUMBER OF UNPACKED AIRCAR (ACARS) REPORTS PROC.  = ',I8,/,35X,
     ...
     $ '   -- NUMBER OF CLASS SOUNDINGS                  = ',I8)
      PRINT 1958, KSKSWN,KSKACF,(IACFTL(II),KSKALM(II),II=1,6),
     $ (IACFTH(II),KSKAHM(II),II=1,6),(KSKNOW(II),II=1,6)
 1958 FORMAT(35X,
     ...
     $ '   -- NO. OF E-ADAS ACFT RPTS BELOW',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF ACARS  ACFT RPTS BELOW',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF TAMDAR ACFT RPTS BELOW',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF CAN-AMDAR   RPTS BELOW',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF AIREP/PIREP RPTS ABOVE',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF AMDAR  ACFT RPTS ABOVE',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF E-ADAS ACFT RPTS ABOVE',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF ACARS  ACFT RPTS ABOVE',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF TAMDAR ACFT RPTS ABOVE',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. OF CAN-AMDAR   RPTS ABOVE',I6,' METERS = ',I8,/,35X,
     $ '   -- NO. AIREP/PIR. RPTS W/MSG WIND & VALID MASS= ',I8,/,35X,
     $ '   -- NO. AMDAR ACFT RPTS W/MSG WIND & VALID MASS= ',I8,/,35X,
     $ '   -- NO. E-ADAS ACFT RPTS W/MSG WND & VALID MASS= ',I8,/,35X,
     $ '   -- NO. ACARS ACFT RPTS W/MSG WIND & VALID MASS= ',I8,/,35X,
     $ '   -- NO. TAMDAR ACT RPTS W/MSG WIND & VALID MASS= ',I8,/,35X,
     $ '   -- NO. CAN-AMDAR  RPTS W/MSG WIND & VALID MASS= ',I8)
      DO  II = 1,2
 ...
 8897 FORMAT(//'  %%  >',I3,' UNIQUE RPT TYPES PROCESSED - COUNTS ',
  ...
  400 FORMAT(//,46X,'REPORT COUNTS BY PREPBUFR REPORT TYPE')
      PRINT 200, (NTYPE(I), I= 1,17),(KTYPE(I), I= 1,17)
      PRINT 200, (NTYPE(I), I=18,34),(KTYPE(I), I=18,34)
      PRINT 200, (NTYPE(I), I=35,51),(KTYPE(I), I=35,51)
      PRINT 200, (NTYPE(I), I=52,68),(KTYPE(I), I=52,68)
      PRINT 200, (NTYPE(I), I=69,NUMTYP-1),(KTYPE(I), I=69,NUMTYP-1)
  200 FORMAT(/,3X,17I7,/,3X,17I7)
      PRINT 203, KKTOT
...
C-----------------------------------------------------------------------
  326 CONTINUE
C PROBLEM IN PRODUCTION DATE FILE -- STOP 29
      PRINT 9326
 9326 FORMAT(/5X,'++ ERROR READING NCEP PRODUCTION DATE FILE- STOP 29'/)
      CALL W3TAGE('PREPOBS_PREPDATA')
      CALL ERREXIT(29)
C-----------------------------------------------------------------------
   99 CONTINUE
...
C     PROCESS); ADDED PROCESSING OF E-ADAS ACFT RPTS NOW POSSIBLY INCL.
...
C  LEVELS THAT CAN BE PROCESSED AND ENCODED INTO BUFR MESSAGES
     ...
     $ KSKALM(6),KSKAHM(6),KSKNOW(6),KSKDRP
      COMMON/MISWCH/XWINDO(6,4),JPSSMI(6,4),SRAD(7),KNTSMI(4),KTIMM(4),
     $ KSKPMI,KMSGMI,KBDYMI
      COMMON/SCTWCH/IEWNDO(2),JPERSD(6),IQWNDO(2),JPQKSD(6),IWWNDO(2),
     $ JPWDSD(6),KSKPSC(4),KMSGSC(4),KBDYSC(4),KTIMSC(4),KNTSCT(4),
...
C  SUBR. GETC06 WILL LATER INIT. ONLY FIRST TWO LEVELS FOR EACH RPT)
      ...
      NFILE = NN + IUNIT(5) - 1
      IF(NN.EQ.2)  THEN
C CHECK TO SEE IF AIRCAR (ACARS) DATA SHOULD BE PROCESSED
...
C CHECK TO SEE IF AIRCFT DATA SHOULD BE PROCESSED
     ...
     $      SUBSKP(004,006).AND.SUBSKP(004,008).AND.SUBSKP(004,012).AND.
...
C IFLAG = 1 RETURNS DATA SET INFO (ONLY) AFTER FIRST CALL
 ...
 8872          FORMAT(5X,A47,6F6.0,' TO ',6F6.0)
  ...
  876          FORMAT(27X,A56,F5.0)
      ...
      NAME2 = 'E-ADAS AIRCFT;    OUT US MNLND/G.MEX/S.ONT.-MINUS & PLUS'
                  PRINT 873, NAME2,AWINDO(1,3)
      NAME2 = 'E-ADAS AIRCFT;    IN  US MNLND/G.MEX/S.ONT.-MINUS & PLUS'
                  PRINT 873, NAME2,AWINDO(2,3)
               END IF
               IF(.NOT.SUBSKP(004,008).OR..NOT.SUBSKP(004,012).OR.
     $            .NOT.SUBSKP(004,013))  THEN
      NAME2 = 'TAMDAR AIRCFT;    OUT US MNLND/G.MEX/S.ONT.-MINUS & PLUS'
                  PRINT 873, NAME2,AWINDO(1,5)
      NAME2 = 'TAMDAR AIRCFT;    IN  US MNLND/G.MEX/S.ONT.-MINUS & PLUS'
                  PRINT 873, NAME2,AWINDO(2,5)
               END IF
               IF(.NOT.SUBSKP(004,009))  THEN
      NAME2 = 'CAN-AMDAR AIRCFT; OUT US MNLND/G.MEX/S.ONT.-MINUS & PLUS'
                  PRINT 873, NAME2,AWINDO(1,6)
      NAME2 = 'CAN-AMDAR AIRCFT; IN  US MNLND/G.MEX/S.ONT.-MINUS & PLUS'
                  PRINT 873, NAME2,AWINDO(2,6)
               END IF
               PRINT'(1X)'
            ELSE  IF(NN.EQ.2)  THEN
      NAME2 = 'ACARS AIRCRAFT;   OUT US MNLND/G.MEX/S.ONT.-MINUS & PLUS'
               PRINT 873, NAME2,AWINDO(1,4)
      NAME2 = 'ACARS AIRCRAFT;   IN  US MNLND/G.MEX/S.ONT.-MINUS & PLUS'
...
C  BECAUSE GFS DUMPS ATE +/-5 HOURS TO IMPROVE PROFCQC PERFORMANCE}
     ...
     $    IDSDAT,IDSDMP_8,SUBSET_d,SUBSKP,IRET)
C.......................................................................
         IF(IRET.EQ.1)  THEN
...
C***********************************************************************
      IFLUA = 0
C-----------------------------------------------------------------------
C CALL SUBR. 'RPTLBL' TO STORE REPORT HEADER INTO HDR(2)-HDR(5),HDR(7),
C                          HDR(9)-HDR(15)
C-----------------------------------------------------------------------
      CALL RPTLBL(CYCLET)
...
C FOR ACARS FILE, BRIEFLY SUMMARIZE IN A TABLE
...
C   CHECK THE DATA SET CENTER DATE AGAINST THE NCEP PRODUCTION DATE.
...
C     CYCLET   - CYCLE TIME (IN HOURS UTC)
C
C   OUTPUT FILES:
C     UNIT 06  - STANDARD OUTPUT PRINT
C
...
C  CRES1, CRES2, OBS2, OBS3 AND NOBS3 HOLD RETURNED REPORT IN SPECIFIED
C  UNPACKED FORMAT {UNLESS FIRST CALL WHICH SHOULD RETURN JUST THE DATA
C  SET INFO (DSNAME, IDSDAT AND IDSDMP_8)}
      OBS2   = BMISS  ! initialize obs2   array before reading any rpts
      OBS3   = BMISS  ! initialize obs3   array before reading any rpts
      NOBS3  = 0      ! initialize nobs3  array before reading any rpts
      RDATA2 = BMISS  ! initialize rdata2 array before reading any rpts
      IF(IW3UNPBF(NFILE,RDATA,STNID,CRES1,CRES2,OBS2,OBS3,NOBS3,DSNAME,
     $ IDSDAT,IDSDMP_8,SUBSET_d,SUBSKP,IER).EQ.0)  GO TO 50
   78 CONTINUE
C-----------------------------------------------------------------------
      IF(IER.EQ.1)  THEN
...
C DOES DATE FROM DATA SET MATCH NCEP PRODUCTION DATE?
  ...
  324 FORMAT(/13X,'>>>>>   P R O B L E M :   D A T E   D O E S   N O T',
     $ '   M A T C H   P R O D U C T I O N   D A T E   <<<<<'/)
            RETURN 1
         END IF
C CYCLET IS TIME THE DATA IS CENTERED ON (CYCLE TIME)
         CYCLET = IDATE(4)
         IF(IOPENED.EQ.1)  THEN
            IOPENED = 0
            IER = 0
...
C   TYPE), 9 (INSTRUMENT TYPE), 10 (ELEVATION), 11 (SEQUENCE NUMBER),
C   12 (ADPUPA REPORT SOLAR AND INFRARED RADIATION CORRECTION
C   INDICATOR), 13 (REPORTED OBS. TIME), 14 (CORRECTION INDICATOR FOR
C   OBS. TIME USED IN REPORT D-TIME CALCULATION), AND 15 (SATELLITE
C   ID) INTO THE REPORT HEADER ARRAY "HDR".  THE STATION ID (IN WORD 1
C   OF HDR), PREPBUFR REPORT TYPE (WORD 6) AND REPORT SUBTYPE (WORD 8)
C   ARE ALL STORED ELSEWHERE.
C
C PROGRAM HISTORY LOG:
...
C    IN REPORT HEADER (CURRENTLY ONLY ACARS STORE SEQ. NO. IN THIS
...
C     FOR HISTORICAL RERUNS); PRINTS THE 4 ACARS TURBULENCE INDECES
C 2001-06-19  D. A. KEYSER -- NEW SUBR. "TIMCHK" DOES TIME WINDOW CHECK
...
C              - REPORT (REAL NUMBER PRECISION)
C
C   OUTPUT FILES:
C     UNIT 06  - STANDARD OUTPUT PRINT
C
...
C LONGITUDE GOES IN HDR(2) (EAST, RANGE: 0.0 TO 359.99)
      HDR(2) = RDATA(2)
C LATITUDE GOES IN HDR(3) (N +, RANGE: -90.00 TO +90.00)
      HDR(3) = RDATA(1)
C DUMP REPORT TYPE GOES IN HDR(7)
      HDR(7)  = IDATA(9)
C REPORT SEQUENCE NUMBER GOES IN HDR(11) (ONLY CURRENTLY FOR ACARS
...
C     DGOT FOR ALL ACFT AND ACARS TYPES
...
C     THAN MM*10; PRINTS ACARS TURB. INDEX; PRINTS GOES SFC PRESS.
C     FOR PW REPORTS
...
C     MARKS (BEFORE SET THEM TO "15"); PRINTS ACARS FLIGHT NUMBER;
...
C CORRECT HGHT LBL TO THICK. TO 1000 MB FOR RTOVS OR ATOVS SAT. SNDINGS
  ...
  915 FORMAT(/'>> (',I6,') ID ',A8,' @',F7.2,' N LAT/',F6.2,' E LON: ',
  ...
  969 FORMAT(5X,'RAIN RATE=',F8.3,' MM/HOUR')
     ...
     $       CLTOP(1,3)
  977 FORMAT(5X,'CLOUD TOP PRESSURE=',F7.1,' MB - Q.M. = ',I2,', CLOUD',
     $ ' COVER=',I3,' %, CLOUD TOP TEMPERATURE=',F6.2)
...
C SINGLE-LEVEL DATA PRINTS HERE
         IF(NFILE.NE.IUNIT(5)+1) PRINT 909, ZSLTT1,TSLTT1,MSLTT1,ZSLTB1,
     $    TSLTB1,MSLTB1,NINT(OBS2(1))
...
cdak     KPIP4 = MIN(15,MOBS(2,4,1))
 ...
 8995 FORMAT(1X,I6,I10,I9,I10,1X,2X,4(1X,I2),2X,4I3.2,'! PRVSTG= ',A8,
  ...
  910 FORMAT(1X,'  PRES ',A10,A9,A10,'   --TBL VALS-   -R. CODES--! '/
     $ 1X,'(*10MB)',A10,A9,A10,14X,'    {MOD(100)}! RESTRICTED FLAG:',
     $ I5,', FLIGHT NUMBER=',A8)
            PRINT 2994, JPIP1,JPIP2,JPIP3,JPIP4,KPIP1,KPIP2,KPIP3,KPIP4,
     $       LPIP1,LPIP2,LPIP3,LPIP4,PCAT,NINT(POAF),
 ...
 2994 FORMAT(1X,I6,I10,I9,I10,1X,2X,4(1X,I2),2X,4I3.2,'! TEMP PREC=',
     $ F5.2,'K; CF: PH FLT=',I1,', TRB INDX(1-4)=',4I2,', MSTQ=',I2)
         ELSE  IF(NFILE.EQ.IUNIT(5)+2)  THEN
            PRINT 8994, JPIP1,JPIP2,JPIP3,JPIP4,KPIP1,KPIP2,KPIP3,KPIP4,
     $       LPIP1,LPIP2,LPIP3,LPIP4,PCAT,NINT(POAF),NINT(TRBX(5)),
     $       NINT(ROLF)
 8994 FORMAT(1X,I6,I10,I9,I10,1X,2X,4(1X,I2),2X,4I3.2,'! T PREC=',F5.2,
     $ 'K; CF: PH FLT=',I1,', TRB IDX=',I2,', ROLL ANG',' FL=',I1)
         ELSE  IF(NFILE.EQ.IUNIT(5)+3.AND.(MIN(RFFL,QIFY,QIFN,EEQF).LT.
     $    YMISS))  THEN
...
C     AND WIND FOR CONV'L/ASDAR AIRCFT COMING FROM RAW PROD AIRCFT
...
C     THE TEST FOR ASDAR AIRCRAFT REPORTS NO LONGER LOOKS FOR AN 'A'
...
C     100 ACARS RPTS HERE
C 1992-12-02  D. A. KEYSER -- IMPR. STD O-PUT (UNIT 6) PRINT RE: CAT. 6
C     MASS/WIND RPTS W/ BAD/MSG DATA; ACFT RPTS W/ BAD/MSG TEMP NO
C     LONGER PROCESS MASS PIECE (PREPDA); ACARS RPTS USE NEW, EXPAND.
C     Q.M. (MARKN), CHGS. SSI/OI Q.M. ON ACARS WIND FROM 2 TO 1
C 1993-05-12  D. A. KEYSER -- SET-UP EXPANDED REPORT FORMAT, WORD 2 FOR
C     EACH MASS/WIND LVL IS PRESS. X 10MB, NEW WORD 5 FOR EACH LEVEL
C     CONTAINS 4 PACKED "PREPBUFR TBL VALUES" WHICH CAN RANGE FROM 0-15
C     AND CORRESPOND TO ON29 Q. MARKERS OR PREPRO-GENERATED Q. MARKERS;
...
C     FLOOR (METERS) FOR ACARS; CONV'L/ASDAR QCACFT WSPD INCR. Q.M. NOW
C     KEPT IN COMMON FOR EA. REPORT; RPT SEQ. NO. NOW STORED IN WORD 12
C     OF HDR FOR CONV'L/ASDAR AIRCFT (=KOUNT)
...
C 1997-06-02  D. A. KEYSER -- ASDAR/AMDAR ACFT REPORTS NOW IDENTIFIED
C     BY LOGICAL SWITCH "AMDAR" (SET IN SUBR. UNPREP) RATHER THAN BY A
C     'Z' IN 6'TH POSITION OF STNID SINCE STNID AT THIS POINT NOW
C     CONTAINS ACTUAL ASDAR/AMDAR STNID (UP TO 8-CHARACTERS, NO LONGER
C     'Z' IN 6'TH POS. OF STNID)
C 1997-10-06  D. A. KEYSER -- ADDED LOGIC TO PROCESS NEW TYPES OF
...
C     OVER CONUS; AMDAR/ASDAR AIRCFT PHASE OF FLT. INDIC. NOW USED TO
C     STORE SPECIAL REPORT SUB-TYPE IN HDR(8)
C 1998-06-15  D. A. KEYSER -- STREAMLINED THE PROCESSING OF CAT. 6
C     RECCOS W/ EITHER MAND LVL GEOPOT. OBS OR MEAN SEA-LVL PRESS. OBS
C 1999-08-04  D. A. KEYSER -- ADDED STORAGE (FOR LATER ENCODING) OF
C     PHASE OF FLIGHT (POAF) FOR ALL ACFT TYPES AND DEGREE OF
C     TURBULENCE (DGOT) FOR NON-ACARS ACFT TYPES; ADDED PROCESSING OF
C     ACARS MOISTURE DATA; FOREIGN SATWND SWITCHES NOW CONTROL IR
...
C 2001-04-06  D. A. KEYSER -- PRINTS DIAG. IF TDEW < TDLIM FOR ACARS;
...
C     ALLOWS THE 2 TO BE DIFFERENT; READS IN THE 4 ACARS TURBULENCE
C     INDECES
C 2001-06-19  D. A. KEYSER -- NEW SUBR. "TIMCHK" DOES TIME WINDOW CHECK
...
C 2004-09-09  D. A. KEYSER -- ADDED PROCESSING OF E-ADAS ACFT RPTS NOW
...
C     ACARS HERE AND TOSS ALL OTHER TYPES DUE TO CONCERN ABOUT
C     PREPACQC), POINT IS MOOT BECAUSE AIFNOW NOW SET TO TRUE TO
C     PROCESS ALL ACFT TYPE TEMP REPORTS WHEN WIND DATA ARE MISSING;
C     NOW FLAGS ALL ACFT REPORTS BELOW "FLOOR" LEVEL (USED TO FLAG ONLY
C     MDCRS ACARS HERE AND TOSS ALL OTHER TYPES DUE TO CONCERN ABOUT
C     PREPACQC); NOW HANDLES SATWINDS FROM NEW INDIAN KALPANA SATELLITE
C     (REPORTS HAVE SUFFIX CHARACTER "K" IN STATION ID); ACCOUNTS FOR
C     EUMETSAT MSG (METEOSAT-8 AND UP) SATWINDS CONTAINING RFF QUALITY
C     INDICATOR; ADDED READING IN OF PERCENT CONFIDENCE BASED ON -
C     EUMETSAT QUALITY INDEX WITH FORECAST CONSISTENCY TEST (FROM CAT.
C     8 CODE TABLE 356), EUMETSAT QUALITY INDEX WITHOUT FORECAST
C     CONSISTENCY TEST (FROM CAT. 8 CODE TABLE 357) AND NESDIS EXPECTED
C     ERROR (FROM CAT. 8 CODE TABLE 358) FOR SATWND TYPES; CASES WHERE
C     OBS ARE SET TO QM=15 IN ORDER TO FLAG A REPORT FOR NON-USE BY
...
C     BE IN REPORT GROUPS FLAGGED FOR NON- USE BY ANALYSIS); THE SINGLE
C     DEGREE OF TURBULENCE VALUE IS NO LONGER RETURNED FROM IW3UNPBF IN
C     CAT. 8, CODE FIGURE 916, IT HAS BEEN REPLACED WITH EXPANDED
C     DEGREE OF TURBULENCE INFORMATION NOW INCLUDED IN THE MULTIPLE-
C     LEVEL SENSIBLE WEATHER ELEMENT ARRAY OBS3 RETURNED FROM IW3UNPBF;
C     AIRCRAFT REPORTS WITH MISSING WIND AND TEMP ARE STILL ENCODED
C     INTO PREPBUFR FILE AS A MASS REPORT IF OTHER ANCILLARY INFO
C     (E.G., SENSIBLE WX ELEMENTS LIKE TURBULENCE, ICING, CLOUD) IS
C     PRESENT (MAINLY PIREPS) AND NEW N-LIST SWITCH WRMISS IS TRUE -
C     BEFORE SUCH OBS WITHOUT ANY WIND AND TEMP DATA WERE ALWAYS
C     SKIPPED
C 2008-09-25  D. A. KEYSER -- 1-DIM ARRAY OBS2 RETURNED FROM IW3UNPBF
C     INCREASED FROM 41 TO 42 WORDS TO HOLD MOISTURE QUALITY (CODE
C     TABLE) FOR ACARS REPORTS; UPPER LIMIT FOR STORING Q CHANGED FROM
C     32767 MG/KG TO 99999 MG/KG TO ENSURE THAT LEVELS WITH VERY
C     HIGH SPECIFIC HUMIDITY ARE PROCESSED (THE 32767 LIMIT WAS A
C     VESTIGE FROM THE OLD I*2 PREPDA DAYS); IF NEAREST INTEGER FOR Q
C     IS 0 MG/KG, IT IS RESET TO 1 MG/KG (.001 G/KG) TO ENSURE THAT
...
C   INPUT ARGUMENT LIST:
C     NN       - UPPER-AIR FILE INDICATOR {=1 ADPUPA/RECCO; =2 ACARS
C              - AIRCRAFT; =3 AIREP/PIREP, AMDAR, E-ADAS, TAMDAR AND
C              - CAN-AMDAR AIRCRAFT; =4 SATELLITE (CLOUD-DRIFT,
C              - W. VAPOR, VISIBLE, ETC)}
C     CYCLET   - CYCLE TIME (IN HOURS UTC)
C
C   OUTPUT FILES:
C     UNIT 06  - STANDARD OUTPUT PRINT
...
C***********************************************************************
      ELSE
C***********************************************************************
C CHECKS ON ACFT FLT-LVL (AIREP/PIREP, AMDAR, E-ADAS, ACARS, TAMDAR,
C  CAN-AMDAR) RPTS (R. TYPE 41)
C***********************************************************************
C-----------------------------------------------------------------------
C DETERMINE AIRCRAFT FLIGHT-LEVEL TYPE (IBNDA,NAC); WHERE: IBNDA IS LAT
C              BAND INDEX AND NAC IS DEFINED AS FOLLOWS:
C   {=1-AIREP/PIREP-PRESET!; =2-AMDAR; =3-E-ADAS; =4-ACARS; =5-TAMDAR;
C                            =6-CAN-AMDAR}
C-----------------------------------------------------------------------
...
C  FLRECO REMAINS FALSE)
      ...
      IF(NN.EQ.3)  THEN
C FOR AIREP/PIREP, AMDAR, E-ADAS, TAMDAR AND CAN-AMDAR AIRCRAFT, STORE
C  WORD 8 OF REPORT HEADER (REPORT "SUBTYPE") AS FOLLOWS:
C   - DEFAULT IS HDR(8) = 0
C   - IF AMDAR, E-ADAS OR CAN-AMDAR AND PHASE OF FLIGHT INDICATOR IS 7
C      (MEANING AIRCRAFT IS "BANKING") STORE HDR(8) = 2 (ABOVE,
C       ICF914=1)
C   - IF AMDAR, E-ADAS OR CAN-AMDAR AND PHASE OF FLIGHT INDICATOR IS
...
C IF PRESSURE ON ANY LVL IS MSG SKIP THIS TOT. RPT (ASSUMES ONLY 1 LVL)
            PRINT 2959, STNID,RDATA(1),RDATA(2),IDATA(9),K
 2959 FORMAT(' * * TOTAL REPORT TOSSED  -ID=',A8,', LAT=',F7.2,'N, ',
     $ 'LON=',F8.2,'E, RTYP',I4,' - PRESS ON LEVEL',I3,' IS MISSING')
            RETURN 2
         END IF
      ENDDO
C***********************************************************************
...
C  AIREP/PIREP, AMDAR, E-ADAS, TAMDAR OR CAN-AMDAR AIRCFT, OR TO
...
C  OR IN OTHER WAYS)
               ...
               ENDDO
               IF(NMOBS2.EQ.0) THEN
                  IF(MIN(ROLF,POAF,TRBX(5),PCAT).LT.YMISS) NMHDR = 1
               END IF
               IF(MAX(NOBS3(2),NOBS3(3),NOBS3(6),NOBS3(7),NMOBS2,NMHDR)
     $          .GT.0) THEN
                  LL = 1
...
C NOW CALL FILLX TO PUT THIS REPORT INTO BLOCK OF REPORTS
 ...
 9787 FORMAT(1X,'* * * NO VALID TEMP/WIND INFO IN ID = ',A8,' ON ANY ',
     ...
     $ 'ELM) DATA * * *')
         ...
         END IF
         KOUNT = KOUNT - 1
         IF(IPRINT.EQ.0)  PRINT 959,STNID,RDATA(1),RDATA(2),IDATA(9)
  959 FORMAT(' * * TOTAL REPORT TOSSED  -ID=',A8,', LAT=',F7.2,'N, ',
     $ 'LON=',F8.2,'E, RTYP',I4,' - NO VALID DATA IN GETC06')
         RETURN 2
      END IF
      END
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
...
C  CHARACTER TO STATION ID
      CALL W3CNVXTOVS(IDATE,IBUF,RDATA,STNID,INSTR,ID1ST5,NN)
C CALL CHKREP TO CHECK FOR MSG LAT/LON & FOR REPORT OUTSIDE SEARCH AREA
...
C ATTEMPT TO DECODE THE NEXT REPORT
     ...
     $ RDATA2,IDSDAT,IDSDMP_8,SUBSET_d,SUBSKP,IRET)
C.......................................................................
      IF(IRET.EQ.1)  THEN
         IFLAG = 1
C IRET = 1 RETURNS DATA SET INFO (ONLY) AFTER FIRST CALL
         ...
         IF(IDATA(27).GT.0)  THEN
            ...
            DO I = 1,IDATA(27)
               ...
               IF(NINT(RDATA(L8+1)).LT.256)  THEN
                  CYCLE
               ELSE  IF(NINT(RDATA(L8+1)).GT.258)  THEN
                  EXIT
...
C   ARRAY.  ASSUMES THAT HDR(2)-HDR(5), HDR(7) AND HDR(9)-HDR(15) HAVE
C   ALREADY BEEN FILLED IN FOR REPORT.
C
C PROGRAM HISTORY LOG:
...
C$$$
      ...
      INTEGER(8)  IDSDMP_8
      REAL OBS2_SAVE(4:42),OBS3_SAVE(5,MXBLVL,7)
      REAL(8)  BMISS
      COMMON/RUNSW/IRNMRK
      COMMON/STRMSL/IPMSL(MXTYPV)
      COMMON/STRMOB/MOBS(MXTYPV,MXWRDL,MXLVL)
     ...
     $ KSKCLS,KSKTHN,KSKOMR(2),KSKALM(6),KSKAHM(6),KSKNOW(6),KSKDRP
      COMMON/SFCBFR/ALTIMR,PRSS
      COMMON/SKPSUB/SUBSKP(0:255,0:200)
      COMMON/DIRECT/OBS3(5,MXBLVL,7),OBS2(42),NOBS3(7),RDATA2(24)
      COMMON /BUFRLIB_MISSING/BMISS
...
C   ITYP = 12 ===> RECCO/DROPWINSONDE WITH NO SPLASH-LEVEL MASS
C                  INFORMATION (INVALID TYPE)
C   ITYP = 13 ===> "OTHER" (INVALID TYPE)
...
C-----------------------------------------------------------------------
C TEST FOR ALL SURFACE MARINE TYPES EXCEPT C-MAN AND AUTOMATED TIDE
C                  GAUGE STNS FOR OVER LAND CRITERIA
C              IF MET SUCH RPTS ARE GIVEN PREPBUFR T.V.=15
C-----------------------------------------------------------------------
C EXCLUDE SFC LAND (+ MESONET), C-MAN AND TIDE GAUGE STNS FROM TEST
      IF(SFLAND.OR.ITYP.EQ.6.OR.ITYP.EQ.9)  GO TO 1490
...
C SKIP WIND PART OF REPORT IF EITHER SURFACE WIND DIR AND/OR SPEED IS
C  MISSING - UNLESS METAR WITH SPEED .LE. 3 M/SEC 
         IF(ITYP.EQ.8.AND.NINT(RDATA(L+3)).LE.30.)  THEN
            IMSG_DIR = 1
         ELSE
            GO TO 1800
         END IF
...
C STORE V-COMP OBS (*100 M/SEC) IN WORD 4 OF MOBS WIND LEVEL 1
         ...
         END IF
      ELSE
C METARS WITH MISSING DIRECTION BUT VALID SPEED COME HERE TO STORE WIND
C  SPEED OBS (*10 M/SEC) IN WORD 6 OF MOBS WIND LEVEL 1
C  NOTE: OTHERWISE WIND DIRECTION AND SPEED ARE NOT STORED IN WORDS 5
C        AND 6 AS THEY ARE FOR OTHER DATA TYPES
         MOBS(1,6,1) = NINT(RDATA(L+3))
      END IF
...
C  WIND REPORT
     ...
     $ STNID,DSNAME,RDATA2,IDSDAT,IDSDMP_8,SUBSET_d,SUBSKP,IRET)
C.......................................................................
      IF(IRET.EQ.1)  THEN
         IFLAG = 1
C IRET = 1 RETURNS DATA SET INFO (ONLY) AFTER FIRST CALL
...
C DOES DATE FROM DATA SET MATCH NCEP PRODUCTION DATE?
  ...
  324 FORMAT(/13X,'>>>>>   P R O B L E M :   D A T E   D O E S   N O T',
     $ '   M A T C H   P R O D U C T I O N   D A T E   <<<<<'/)
            GO TO 110
         END IF
C CYCLET IS TIME THE GPS-IPW DATA IS CENTERED ON (CYCLE TIME)
         CYCLET = IDATE(4)
         PRINT 871
  871 FORMAT(/42X,'+++  TIME WINDOW  (HRS X 100) ON REPORTS  +++'/)
         PRINT 875, GWINDO
...
C    (CONV'L/ASDAR AIRCFT WIND REPORTS ONLY) ENCODED INTO REPORT HEADER
...
C    AIRCRAFT ASDAR/AMDAR REPORTS
C 1996-09-03  D. A. KEYSER -- PROCESSING OF TOVS RADIANCES PARTIALLY
C    DONE (SWITCH "TOVRAD")
...
C    WASTED LOTS OF SPACE; ADDED ENCODING OF PHASE OF FLIGHT ('POAF')
C    FOR ALL ACFT TYPES AND DEGREE OF TURBULENCE (DGOT) FOR NON-ACARS
C    ACFT TYPES; ADDED PROCESSING OF ACARS MOISTURE DATA; RADIOSONDE
...
C     ACARS TURBULENCE INDECES INTO THE PREPBUFR FILE UNDER MNEMONICS
...
C     AND "AIRCFT" (ONLY AVAIL. FOR AMDAR/ASDAR AND ACARS)
...
C     "RASSDA"; NOW ENCODES ACARS FLIGHT NUMBER (MNEMONIC "ACID") INTO
C     MDCRS ACARS REPORTS; ADDED PROCESSING OF COOPERATIVE AGENCY AND
C     JAPANESE WIND PROFILERS NOW POSSIBLY INCL. IN "PROFLR" DUMP FILE
C     DUMP R. TYPES 75 AND 76, RESP., GETS PREPBUFR R. TYPES 227 AND
C     228, RESP., PROCESSED INTO PREPBUFR FILE UNDER EXISTING TBL A
C     ENTRY "PROFLR", WIND PROFILER RPTS ORIGINATING FROM PILOT (PIBAL)
C     BULLETINS NOW GET A UNIQUE PREPBUFR R. TYPE OF 229; ADDED LOGIC
C     (CURRENTLY NOT INVOKED) TO SEPARATE METAR REPORTS INTO THEIR
C     OWN MESSAGES IN THE ADPSFC MESSAGE TYPE, BUT WITH A UNIQUE
C     MESSAGE SUB-TYPE OF 7 (SEE COMMENTS IN SUBROUTINE); SST AND
C     SENSIBLE WEATHER ELEMENTS (IN OBS2 AND OBS3 ARRAYS RETURNED FROM
C     IW3UNPBF) ENCODED INTO PREPBUFR FILE (MANY NEW MNEMONICS) BUT
C     ONLY IN APPROPRIATE MESSAGE TYPES
...
C     E-ADAS, CANADIAN AMDAR, TAMDAR), ENCODED INTO PREPBUFR FILE IN
...
C   HHDR(3) = IMISS (99999) THIS INDICATES THAT ALL PREPROCESSED REPORTS
...
C  TABLE-A MESSAGE TYPES
      ...
      COMMON/FIZZAA/PCODE,IUNITP,NLV,NBT,HDR(MHDRWD),OBS(MOBSWD,MXBLVL),
     $ OB2(MOB2WD,MXBLVL),QMS(MQMSWD,MXBLVL),PGM(MPGMWD,MXBLVL),
     $ RSN(MPGMWD,MXBLVL),BTO(2,MXBLVL),RAD(5),IUNITG(2)
      COMMON/UNITNO/NFILE,IUNIT(28)
      COMMON/XTRHD2/CRES1,CRES2
      COMMON/DATA/DMY3,IDATE(4),IDAT10,MP_PROCESS,WRMISS
      COMMON/RPTHDR/SUBSET_d,STNID,HHDR(2:MXWRDH)
      COMMON/PREVSW/PREVEN
      COMMON/APDNSW/APPEND
      COMMON/DIRECT/OBS3(5,MXBLVL,7),OBS2(42),NOBS3(7),RDATA2(24)
     ...
     $ SUBSET_d
      ...
      REAL(8) HDR_8(MHDRWD),OBS_8(MOBSWD,MXBLVL),OB2_8(MOB2WD,MXBLVL),
     $ QMS_8(MQMSWD,MXBLVL),PGM_8(MPGMWD,MXBLVL),RSN_8(MPGMWD,MXBLVL),
     $ BTO_8(2,MXBLVL),RAD_8(5),AERS_8(12),OBS2_8(42),
     $ OBS3_8(5,MXBLVL,7),RPRV_8(2),RACID_8,RDATA2_8(24)
...
C  MNEMONICS PASSED INTO SUBROUTINE "UFBINT".
     ...
     $     'POB UOB  VOB  TOB QOB ZOB CAT TVO TDO NUL  SOB  NUL    ',
     $     'POB UOB  VOB  TOB QOB ZOB CAT TVO TDO DDO  FFO  NUL    ',
     ...
     $     'POB UOB  VOB  NUL NUL NUL CAT NUL NUL DDO  FFO  NUL    ',
     $     'POB UOB  VOB  TOB QOB ZOB CAT TVO TDO NUL  NUL  NUL    ',
     ...
     $     ' PMO    ALSE NUL   NUL    NUL    NUL                   ',
     $     ' PMO    NUL  NUL   NUL    NUL    NUL                   ',
     $     ' NUL    NUL  NUL   NUL    NUL    NUL                   ',
     $     ' NUL    NUL  NUL   NUL    NUL    NUL                   ',
...
C-----------------------------------------------------------------------
      END IF
      KI  = HHDR(6)/100
      NI  = MOD((NINT(HHDR(6))/10),10)
      JI  = MOD(NINT(HHDR(6)),10)
      CTYPE = 'MASS'
...
C            "SFCSHP" INPUT FILE)
C-----------------------------------------------------------------------
      ITYP = KTYP(NINT(HHDR(6)))
      IF(NFILE.EQ.IUNIT(6)+1)  ITYP = 10
      IF(ITYP.LE.0)  THEN
C NOTE REPORTS WITH A PREPBUFR REPORT TYPE THAT IS NOT RECOGNIZED HERE
C  -- SKIP
         PRINT 113, CTYPE,STNID,HHDR(3),HHDR(2),NINT(HHDR(6))
  113 FORMAT(' + + ',A4,' REPORT TOSSED   -ID=',A8,', LAT=',F7.2,'N, ',
     $'LON=',F8.2,'E, RTYP',I4,' - NOT RECOGNIZED IN SUBROUTINE W3FIZZ')
         GO TO 999
      END IF
C-----------------------------------------------------------------------
...
C  sub-type of 7 - this was going to be done to order to speed up the
...
C-----------------------------------------------------------------------
C STORE THE STATION ID ("HDR" WORD 1)
         HDR_8=HDR
         CHDR = STNID
...
C.......................................................................
            ...
            END IF
            IF(HHDR(6).EQ.133.OR.HHDR(6).EQ.233)  THEN
C.......................................................................
C           ACARS REPORTS ENCODE FLIGHT NUMBER (CHARACTER*8)
C.......................................................................
               CACID = CRES2
               CALL UFBINT(IUNITP,RACID_8,1,1,IRET,'ACID')
            END IF
            IF(HHDR(6).EQ.134.OR.HHDR(6).EQ.234)  THEN
C.......................................................................
C    TAMDAR REPORTS ENCODE FLIGHT NUMBER (SAME AS STNID) (CHARACTER*8)
C.......................................................................
               CACID = STNID
               CALL UFBINT(IUNITP,RACID_8,1,1,IRET,'ACID')
            END IF
...
C NOTE THAT THIS REPORT (SUBSET) NOT PROCESSED DUE TO NO. LEVELS = 0
         PRINT 213, CTYPE,STNID,HHDR(3),HHDR(2),NINT(HHDR(6))
  213 FORMAT(' + + ',A4,' REPORT TOSSED   -ID=',A8,', LAT=',F7.2,'N, ',
     $ 'LON=',F8.2,'E, RTYP',I4,' - ZERO LEVELS PROCESSED IN ',
     $ 'SUBROUTINE W3FIZZ')
      END IF
...
C     AND QCACFT WIND SPEED INCR. Q.M. (CONV'L/ASDAR AIRCFT WIND
...
C 1997-10-06  D. A. KEYSER -- ENCODES FOV# AS 'ACAV' INTO PREPBUFR FOR
C     GOES REPORTS
C 1998-02-17  D. A. KEYSER -- RECEIPT TIME NOW READ FROM HHDR(5)
...
C     WASTED LOTS OF SPACE; ADDED ENCODING OF PHASE OF FLIGHT ('POAF')
C     FOR ALL ACFT TYPES AND DEGREE OF TURBULENCE (DGOT) FOR NON-ACARS
C     ACFT TYPES; ADDED PROCESSING OF ACARS MOISTURE DATA BUT THEY ARE
...
C     (DEF.) FLAGS ALL ACARS MOISTURE DATA (WAS HARDWIRED AS FLAGGED);
...
C     ENCODES THE 4 ACARS TURBULENCE INDECES INTO THE PREPBUFR FILE
...
C     QUALITY (CODE TABLE) FOR ACARS REPORTS, ENCODED INTO PREPBUFR
...
C$$$
     ...
     $ FLAGM3,FLAGM4,FLAGM5,FLDMGS,FLDMAF,FLDMFR,FLACMS,GOESRD,FLRECO
...
C FOR SUBSET = SATWND .....
         IF(RFFL.LT.YMISS)  HDR(16) = RFFL
         IF(QIFY.LT.YMISS)  HDR(27) = QIFY
         IF(QIFN.LT.YMISS)  HDR(28) = QIFN
         IF(EEQF.LT.YMISS)  HDR(29) = EEQF
      END IF
      HDR(6) = HHDR(6)
      HDR(7) = HHDR(7)
      IF(HHDR(8).LT.YMISS)  HDR(8) = HHDR(8)
      HDR(9) = HHDR(9)
...
ccdak IF(HHDR(11).LT.YMISS)  HDR(11) = HHDR(11)
      ...
      HDR(17) = HHDR(13)
      HDR(18) = HHDR(14)
      IF(HHDR(15).LT.YMISS)  HDR(21) = HHDR(15)
      HDR(22) = MP_PROCESS
...
C DATA LEVEL CATEGORY IS STORED AS 6 (NOT DEFINED IN ICAT ARRAY)
         OBS(7,1) = 6
         IF(HHDR(6).EQ.152.OR.HHDR(6).EQ.153)  THEN
...
C STORE CLOUD TOP PRESSURE OBS., TABLE VALUE, PROGRAM CODE & REASON CODE
         ...
         PGM(9,1)  = PCODE
         RSN(9,1)  = 100.
C STORE CLOUD COVER OBS. AND CLOUD TOP TEMPERATURE OBS. (THESE CAN BE
C  MISSING)
         IF(CLTOP(1,2).LT.YMISS)  OB2(2,1) = CLTOP(1,2)
         IF(CLTOP(1,3).LT.YMISS)  OB2(3,1) = CLTOP(1,3)
         NLV = 1

      ELSE  IF(HHDR(6).EQ.102)  THEN
C***********************************************************************
...
C --> PROCESSING UNIQUE TO FIRST MOBS LEVEL
               ...
               IF(IPMSL(1).LT.YMISS)  THEN
C STORE MEAN SEA-LEVEL PRESSURE OBS. AND TABLE VALUE
...
C  MISSING
               ...
               ELSE  IF(HHDR(6).EQ.120.OR.HHDR(6).EQ.220.OR.
     $                  HHDR(6).EQ.221)  THEN
...
C  (NUMERIC) AND TURBULENCE INDEX (NUMERIC), IF NON-MISSING
                     IF(HHDR(5).LT.YMISS)  OBS(12,NLV) = HHDR(5)
...
C.......................................................................
                  ...
                  ELSE  IF(SUBSET.EQ.'ADPSFC  '.AND.OBS5.GE.YMISS.AND.
     $             OBS6.LT.YMISS)  THEN
C.......................................................................
C FOR SUBSET = ADPSFC, IF WIND SPEED IS PRESENT AND WIND DIRECTION IS
C  MISSING, THIS IS A METAR REPORT ORIGINALLY REPORTING VARIABLE
C  WIND DIRECTION AND WIND SPEED .LE. 3 M/SEC; STORE WIND SPEED (M/SEC)
C  OBS AND TABLE VALUE
                        OBS(11,NLV) = OBS6 * 0.1
                        QMS(6,NLV)  = QMS4
C.......................................................................
                  END IF
...
C      HERE
                           ...
                           END IF
                        END IF
                     ELSE  IF(HHDR(6).EQ.133)  THEN
                        FLAGM3 = (FLACMS(4))
                     ELSE  IF(HHDR(6).EQ.134)  THEN
                        FLAGM3 = (FLACMS(5))
                     END IF
...
C IF FLDMFR=T, NON-US OR UNKNOWN DROPS GET MOISTURE FLAGGED ON ALL LVLS
                        ...
                        IF(FLAGM1)  THEN
                           IF(IFRST1.EQ.1)
     $                     PRINT 213,STNID,HHDR(3),HHDR(2),NINT(HHDR(6))
  213 FORMAT(' + + ALL Q LVLS FLAGGED   -ID=',A8,', LAT=',F7.2,'N, ',
     $ 'LON=',F8.2,'E, RTYP',I4,' - NOAA (P-3 OR GULF STREAM) ',
     $ 'DROPWINSONDE')
                           IFRST1 = 0
                        ELSE  IF(FLAGM2)  THEN
                           IF(IFRST2.EQ.1)
     $                     PRINT 293,STNID,HHDR(3),HHDR(2),NINT(HHDR(6))
  293 FORMAT(' + + ALL Q LVLS FLAGGED   -ID=',A8,', LAT=',F7.2,'N, ',
     $ 'LON=',F8.2,'E, RTYP',I4,' - USAF DROPWINSONDE')
                           IFRST2 = 0
                        ELSE  IF(FLAGM5)  THEN
                           IF(IFRST3.EQ.1)
     $                     PRINT 294,STNID,HHDR(3),HHDR(2),NINT(HHDR(6))
  294 FORMAT(' + + ALL Q LVLS FLAGGED   -ID=',A8,', LAT=',F7.2,'N, ',
     $ 'LON=',F8.2,'E, RTYP',I4,' - NON-US OR UNKNOWN DROPWINSONDE')
                           IFRST3 = 0
                        END IF
                     ELSE  IF(FLAGM3)  THEN
...
C  GET MOISTURE FLAGGED
                        IF(QMS4.LE.3.OR.QMS4.GT.15) QMS4 = 15
                        PRINT 8213, STNID,HHDR(3),HHDR(2),
     $                   NINT(HHDR(6))
 8213 FORMAT(' + + SP. HUMIDITY FLAGGED -ID=',A8,', LAT=',F7.2,'N, ',
...
C IF DEWPOINT (K) .LT. TDLIM, MOISTURE FLAGGED FOR ALL DATA TYPES
 ...
 8214 FORMAT(' + + SP. HUMIDITY FLAGGED -ID=',A8,', LAT=',F7.2,'N, ',
     $ 'LON=',F8.2,'E, RTYP',I4,', PRESS=',F6.2,'MB - TD (',F5.1,') < ',
     $ 'TDLIM (',F5.1,')')
                     END IF
C. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
                     OBS(5,NLV) = OBS4
...
C   PRGMMR: D. A. KEYSER     ORG: NP22       DATE: 1988-04-11
C
...
C$$$
     ...
     $ TOVRTV/.FALSE./,AIFNOW/6*.FALSE./,FLDMGS/.TRUE./,
     $ FLACMS/6*.TRUE./,GOESRD/2*.FALSE./,PREVEN/.FALSE./,
     $ FLDMAF/.TRUE./,FLDMFR/.TRUE./,DROPSN/.TRUE./,
     $ SUBSKP/51456*.FALSE./,APPEND/.FALSE./,PROFILERinADPUPA/.FALSE./,
     $ RASS/.FALSE./,WRMISS/.TRUE./,SKGP45/.TRUE./
     ...
     $ FWINDO/4*300.,600.,6*300./,RWINDO/300./,UWINDO/300./,
     $ AWINDO/7*300.,150.,4*300./,SWINDO_e/12*-300.,6*-700.,6*-300.,
     $ 12*-300.,12*-300.,12*-300.,6*-700.,6*-300.,12*-300.,12*-300.,
     $ 12*-300.,6*-700.,6*-300.,12*-300.,12*-300.,336*-300./,SWINDO_l/
     $ 12*+300.,6*+700.,6*+300.,12*+300.,12*+300.,12*+300.,6*+700.,
     $ 6*+300.,12*+300.,12*+300.,12*+300.,6*+700.,6*+300.,12*+300.,
     $ 12*+300.,336*+300./,XWINDO/24*300./,JPSSMI/24*9999/,IEWNDO/-3,3/,
     $ JPERSD/6*9999/,PWINDO/4*300./,JPQKSD/6*9999/,JPWDSD/6*9999/,
     $ IQWNDO/-3,3/,IWWNDO/-3,3/,CWINDO/300./,VWINDO/300./,DWINDO/300./,
     $ JPGPSD/6*9999/,GWINDO/300./,TWINDO/300./,JPASCD/6*9999/,
     $ IAWNDO/-3,3/
     ...
     $ ISATOB/0/,IPRINT/0/,ISATLS/80*2/
      DATA  JAWIND/36*0/,JAMASS/36*9999/,JSURFM/11*0/,
     $      JSURFW/2*0,9999,0,9999,0,9999,4*0/,
     ...
     $      IACFTL/100,5*2286/,IACFTH/3*18000,16500,2*18000/,
     $      LSMASS/9999/,LSWIND/9999/,KEPAC/36*0/,IGCHN/18*IMISS/,
     ...
     $      ROLF/XMISS/,QIFY/XMISS/,QIFN/XMISS/,EEQF/XMISS/,aMSTQ/XMISS/
      DATA  LFMAXI/53/,LFMAXJ/45/,FMESHL/190.5/,FLONVT/105./,FPOLEI/27./
     $,     FPOLEJ/49./,LEVPM/16/,LEVST/13/,LEVQQ/8/,TDLIM/215./,
     $      ISQNUM_UPA/0/
     ...
     $      KIPTTV/2*0/,KSKTHN/0/,KJFLAG/2*0/,KSKOMR/2*0/,KSKALM/6*0/,
