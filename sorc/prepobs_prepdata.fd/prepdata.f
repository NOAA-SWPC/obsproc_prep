...
C   PRGMMR: KEYSER                ORG: NP22   DATE: 2015-04-08
C
...
C  IN INTERFACE WITH SUBROUTINE IW3UNPBF
     ...
     $ FLDMFR,WRMISS,SKGP45,JPASCD,IAWNDO,npkrpt
      NAMELIST/PARM/IUNIT
      CALL W3TAGB('PREPOBS_PREPDATA',2015,0098,0061,'NP22')
...
C    CARDS FILE, JUST AFTER NAMELIST TASK, BY THE MAKE_PREPBUFR SCRIPT)
  ...
  321 FORMAT(/37X,'WELCOME TO THE UNIVERSAL ',A14,' DATA PREPROCESSOR'/
     $ 48X,'WCOSS VERSION CREATED 08 APR 2015'/)
...
C   PRGMMR: D. A. KEYSER     ORG: NP22       DATE: 2015-03-09
C
...
C  CRES1, CRES2, CBULL, OBS2, OBS3 AND NOBS3 HOLD RETURNED REPORT IN
C  SPECIFIED UNPACKED FORMAT {UNLESS FIRST CALL WHICH SHOULD RETURN
C  JUST THE DATA SET INFO (DSNAME, IDSDAT AND IDSDMP_8)}
      OBS2   = BMISS  ! initialize obs2   array before reading any rpts
      OBS3   = BMISS  ! initialize obs3   array before reading any rpts
      NOBS3  = 0      ! initialize nobs3  array before reading any rpts
      RDATA2 = BMISS  ! initialize rdata2 array before reading any rpts
      IF(IW3UNPBF(NFILE,RDATA,STNID,CRES1,CRES2,CBULL,OBS2,OBS3,NOBS3,
     $ DSNAME,IDSDAT,IDSDMP_8,SUBSET_d,SUBSKP,IER).EQ.0)  GO TO 50
   78 CONTINUE
C-----------------------------------------------------------------------
      IF(IER.EQ.1)  THEN
...
C
C SUBPROGRAM:    RPTLBL
C   PRGMMR: D. A. KEYSER     ORG: W/NMC22    DATE: 2015-04-08
C
...
C ACARS REPORT SEQUENCE NUMBER FOUND
         ...
         END IF
  856    CONTINUE
C DUMP REPORT SUBTYPE GOES IN HDR(8) (for ACARS, based on WMO bulletin
...
C  WORDS 2-29 (NOTE: SOME WORDS HERE ARE SKIPPED)}
      ...
      NSCL0 = 2
cvvvvvvvv
c After linking to new BUFRLIB v11.0.0., can add the following to obtain
c  NSCL0 - can then remove comment after "HDR(2) = 0.0" below
ccccc call NEMSPECS(iunitp,'XOB',1,NSCL, NREF, NBTS, IRET)
ccccc if(IRET.eq.0)  NSCL0 = NSCL
c^^^^^^^^
      IF(NINT(HDR(2)*100).EQ.360*10**NSCL0) HDR(2) = 0.0 !change this if
                                                         !XOB precision
                                                         !ever changes
                                                         !in BUFR table 
      HDR(3) = HHDR(3)
...
C$$$
     ...
     $ ISATOB/0/,IPRINT/0/,ISATLS/80*2/
      DATA  JAWIND/36*0/,JAMASS/36*9999/,JSURFM/12*0/,
